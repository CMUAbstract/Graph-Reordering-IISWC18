...
pvector<ScoreT> Brandes(const Graph &g, SourcePicker<Graph> &sp,
                        NodeID num_iters, pvector<NodeID> &new_ids) {
  Timer t;
  t.Start();
  ...
  for (NodeID iter=0; iter < num_iters; iter++) {
    NodeID source = new_ids[sp.PickNext()];
    t.Stop();
...
bool BCVerifier(const Graph &g, SourcePicker<Graph> &sp, NodeID num_iters,
                const pvector<ScoreT> &scores_to_test, pvector<NodeID> &new_ids) {
  pvector<ScoreT> scores(g.num_nodes(), 0);
...
int main(int argc, char* argv[]) {
  ...
  Graph sorted_g =  Builder::degreeSort(g, (cli.relabel() == 0), new_ids, false, false);
  SourcePicker<Graph> sp(g, cli.start_vertex());
  auto BCBound =
    [&sp, &cli, &new_ids] (const Graph &sorted_g) { return Brandes(sorted_g, sp, cli.num_iters(), new_ids); };
  SourcePicker<Graph> vsp(g, cli.start_vertex());
  auto VerifierBound = [&vsp, &cli, &new_ids] (const Graph &sorted_g,
                                     const pvector<ScoreT> &scores) {
    return BCVerifier(sorted_g, vsp, cli.num_iters(), scores, new_ids);
  };
  BenchmarkKernel(cli, sorted_g, BCBound, PrintTopScores, VerifierBound, new_ids);
  return 0;
}
