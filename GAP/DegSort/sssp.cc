...
pvector<WeightT> DeltaStep(const WGraph &g, NodeID source, WeightT delta) {
  ...
  #pragma omp parallel
  {
    ...
    while (shared_indexes[iter&1] != kMaxBin) {
      ...
      {
        t.Stop();
        PrintStep(curr_bin_index, t.Seconds(), curr_frontier_tail);
        t.Start();
...
void PrintSSSPStats(const WGraph &g, const pvector<WeightT> &dist, pvector<NodeID> &newIds) {
...
int main(int argc, char* argv[]) {
  ...
  auto SSSPBound = [&sp, &cli, &new_ids] (const WGraph &sorted_g) {
    return DeltaStep(sorted_g, new_ids[sp.PickNext()], cli.delta());
  };
  ...
  BenchmarkKernel(cli, sorted_g, SSSPBound, PrintSSSPStats, VerifierBound, new_ids);
  return 0;
}
