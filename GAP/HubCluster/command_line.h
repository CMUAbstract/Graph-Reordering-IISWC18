...
class CLBase {
 protected:
  ...
  char** argv_;
  std::string name_;
  std::string get_args_ = "f:g:hk:su:r:l:x:";
  ...
  bool uniform_ = false;
  /* relabel_ encodes whether we should relabel the graph using in- or out-degrees
  0 - out-degrees
  1 - in-degrees */
  int relabel_ = 0; 
  int cutoffFactor_ = 1;
 ...
 public:
  CLBase(int argc, char** argv, std::string name = "") :
    ...
    AddHelpLine('l', "relabel-direction", "relabel graph using outdegrees (0) or indegrees(1)");
    AddHelpLine('x', "Cutoff factor", "degreeThreshold = cutoffFactor * avgDegree");
  }
  ...
  void virtual HandleArg(signed char opt, char* opt_arg) {
    switch (opt) {
      ...
      case 'x': cutoffFactor_ = atoi(opt_arg);              break;
    }
  }
  ...
  int cutoffFactor() const { return cutoffFactor_; }
};
